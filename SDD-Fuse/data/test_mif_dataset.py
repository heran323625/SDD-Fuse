...
class Test_Dataset(Dataset):
    def __init__(self, dataroot, resolution=256, split='val', data_len=-1):
        self.resolution = resolution
        self.data_len = data_len
        self.split = split
        self.CT_path = Util.get_paths_from_images('{}/CT-PET-SPECT'.format(dataroot))
        self.MRI_path = Util.get_paths_from_images('{}/MRI'.format(dataroot))

        self.dataset_len = len(self.CT_path)
    ...
    def __getitem__(self, index):
        img_vis = Image.open(self.CT_path[index]).convert("YCbCr")
        img_ir = Image.open(self.MRI_path[index]).convert("YCbCr")

        img_full = Image.open(self.CT_path[index]).convert("RGB")
